...
def predict_crop_route():
    data = request.json
    try:
        crop, explanation = predict_crop(float(data["nitrogen"]), float(data["phosphorus"]), 
                                         float(data["potassium"]), float(data["ph"]), 
                                         float(data["humidity"]))
        return jsonify({"crop": crop, "explanation": explanation})
    except Exception as e:
        print(f"API Error: {e}")
